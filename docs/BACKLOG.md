...
_Last updated: 2026-02-20 (batch run: B11-001–003, B11-005–006, B11-009, B11-015, B11-017, B11-019 DONE; second batch B11-018, B11-004, B11-007, B11-010, B11-011, B11-008 DONE; third batch B11-012, B11-013, B11-014 DONE; B11-016 DONE). See `docs/PHASE_11_AUDIT.md` for full rationale._
...
| B11-004 | Delete pre-Memory OS planned workout stack | Cleanup | `src/planned_workout_manager.py`, `src/planned_workout_cli.py`, `src/extract_baseline_plan.py`, `bin/planned_workouts.sh` | Files deleted; `pytest tests/` passes; no callers exist | M | S | **DONE** | B11-003 | Deleted 2026-02-20; zero callers confirmed outside the stack; 73 tests pass |
...
| B11-007 | Add `--days` and `--check-only` to `coach sync` | Enhancement | `cli/coach.py`, `skills/garmin_sync.py` | `coach sync --days 7` and `coach sync --check-only` work; tests pass | M | M | **DONE** | — | Added 2026-02-20; extracted `_build_parser()`; `run()` routes --days/--check-only to sync_garmin_data.sh; `tests/test_sync_args.py` (9 tests); 82 pass |
| B11-008 | Move cache-age check from bash into Python | Refactor | `skills/garmin_sync.py`, `bin/smart_sync.sh` | `skills/garmin_sync.run(max_age_minutes=30)` implements age check natively; `smart_sync.sh` kept as thin shim calling `coach sync`; test added | M | M | **DONE** | B11-007 | Done 2026-02-20; `_cache_age_minutes()` in Python; `smart_sync.sh` → thin shim; `skipped` key in result; 11 tests in `test_sync_args.py`; 99 pass |
...
| B11-010 | Add `daily_metrics` table to SQLite schema | Schema | `memory/db.py` | `daily_metrics` table created via `init_db()`; migration is additive (no existing data dropped); `pytest tests/` passes | M | M | **DONE** | — | Added 2026-02-20; typed columns + raw_json; helpers upsert/get_daily_metrics; `tests/test_daily_metrics.py` (7 tests); 89 pass |
| B11-011 | Add `activities` table to SQLite schema | Schema | `memory/db.py` | `activities` table created via `init_db()`; migration additive; `pytest tests/` passes | M | M | **DONE** | B11-010 | Added 2026-02-20; typed columns + raw_json; helpers upsert_activity/get_activities; `tests/test_activities.py` (8 tests); 97 pass |
| B11-012 | Wire post-sync SQLite ingest (daily metrics) | Integration | `skills/garmin_sync.py`, `memory/db.py` | After successful sync, `daily_metrics` rows inserted/updated for synced date range; smoke test shows rows present after `coach sync` | M | M | **DONE** | B11-010, B11-008 | Done 2026-02-20; `_ingest_daily_metrics()` in garmin_sync.py; maps hrv/sleep/BB/TR/RHR/stress; `tests/test_ingest.py` (17 tests); 116 pass |
| B11-013 | Wire post-sync SQLite ingest (activities) | Integration | `skills/garmin_sync.py`, `memory/db.py` | After successful sync, `activities` rows inserted/updated; smoke test shows rows present | M | M | **DONE** | B11-011, B11-012 | Done 2026-02-20; `_ingest_activities()` in garmin_sync.py; converts miles→m, min/mile→s/mile; idempotent; 116 pass |
| B11-014 | Update `memory/retrieval.py` to read daily_metrics from SQLite | Migration | `memory/retrieval.py` | Context packet reads recovery metrics from `daily_metrics` table; falls back to JSON cache if table empty; unit test added | H | M | **DONE** | B11-012 | Done 2026-02-20; `_rollup_readiness_from_sqlite()` added; SQLite-first with JSON fallback; 4 tests in `test_retrieval.py`; 120 pass |
...
| B11-018 | Make schedule output mobile-first by default | UX | `cli/coach.py`, `src/discord_bot.py`, possibly schedule formatter | `/coach_schedule` defaults to compact/mobile format; long lines wrapped; no horizontal scrolling needed on mobile; table format still available via `--format table` | L | S | **DONE** | — | Changed argparse default + getattr fallback from "table" → "mobile" in `cli/coach.py`; Discord already defaulted to mobile; 73 tests pass |
| B11-019 | Add `sync_runs` table + freshness visibility | Observability / Sync provenance | `memory/db.py`, `skills/garmin_sync.py`, `cli/coach.py`, `src/discord_bot.py` (and tests as needed) | (1) New SQLite table `sync_runs` created by `init_db()` (additive; no existing tables altered/dropped). (2) Every `coach sync` writes a row with: `started_at`, `finished_at`, `status`, `source`, `days_requested`, `days_synced`, `error_summary` (nullable), `run_id`. (3) `coach agent status` displays: last successful sync timestamp, age in minutes, last run status + error summary if failed. (4) Discord `/coach_status` embed includes same freshness fields. (5) Tests added for table creation + at least one "sync run recorded" path (mocking ok). | M | M | **DONE** | — | Added `sync_runs` table 2026-02-20; helpers in `memory/db.py`; wired in `skills/garmin_sync.run(source=)`; freshness in `_agent_status()`; Discord routes through CLI so gets it free; 10 tests; 73 pass |
...
| B11-019 | Add `sync_runs` table + freshness visibility | 2026-02-20 | Table + helpers in `memory/db.py`; wired in `skills/garmin_sync.run()`; freshness in `_agent_status()`; `tests/test_sync_runs.py` (10 tests); 73 tests pass |
